...
export class MMU {
  constructor(name, algorithm, operations) {
    ...
    this.internalWaste = 0;
    this.runningPids = new Set();
    this.clockHand = 0;
  }
  ...
  new(pid, sizeBytes, opIndex) {
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      this.clock += FAULT_TIME;
      this.thrashing += FAULT_TIME;
    }
  ...
  use(ptr, opIndex) {
    const rec = this.pointers.get(ptr);
    if (!rec)
       return { event: "invalid-use", ptr };
  ...
  delete(ptr) {
    const rec = this.pointers.get(ptr);
    if (!rec) 
      return { event: "invalid-delete", ptr };
  ...
  kill(pid) {
    for (const [ptr, rec] of [...this.pointers])
       if (rec.pid === pid)
         this.delete(ptr);
  ...
  placeInRam(page, opIndex) {
    ...
    if (frame === -1) {
      ...
      const victim = this.ram[frame];
      victim.inRam = false; 
      victim.frame = null;
      this.vram.set(victim.id, victim);
    }
    this.vram.delete(page.id);
    page.inRam = true;
    page.frame = frame;
    page.loadedAt = this.clock;
    this.ram[frame] = page;
  }
